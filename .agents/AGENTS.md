...
23. **The AGENTS.md Parity & Sync Mandate:** The AI and human operators must strictly maintain parity and keep both `AGENTS.md` (at the repository root) and `.agents/AGENTS.md` synchronized and up to date. The root `AGENTS.md` serves as a routing gateway notifying external AI agents (like Google Jules) to read `.agents/AGENTS.md` and explore `.agents/brain/` files, while `.agents/AGENTS.md` contains the full cognitive ruleset. Keeping both updated ensures effective workspace alignment.
